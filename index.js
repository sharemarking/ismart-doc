...
// 读取config内容
try {
    config = require(path.join(process.cwd(), configPath)).doc;
} catch (e) {
...
// 排除目录矫正
_.each(config.skip, function (v) {
    skipDir.push(path.join(srcPath, v))
});
...
!fs.existsSync(path.join(distPath, 'javascripts')) && fs.mkdirSync(path.join(distPath, 'javascripts'));
...
// 生成目标js，便于控制台即调用
if (config.source) {
    fs.writeFileSync(path.join(distPath, 'javascripts', config.name + '.js'), fs.readFileSync(config.source));
}
